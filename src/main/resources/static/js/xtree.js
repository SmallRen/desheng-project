...
layuiXtree.prototype.getByClassName = function(cn) {
    if (document.getElementsByClassName)
        return this._container.getElementsByClassName(cn);
...
layuiXtree.prototype.Loading = function(options) {
    var _this = this;
    _this.xloading = document.createElement("span");
    // 创建加载对象
    ...
    _this._container.appendChild(_this.xloading);
    // 加载显示
    ...
    obj.onreadystatechange = function() {
        if (obj.readyState == 4 && obj.status == 200 || obj.status == 304) {
            // 回调成功
            _this._dataJson = eval('(' + obj.responseText + ')');
            // 将返回的数据转为json
            _this.Initial(options);
        }
    }
...
layuiXtree.prototype.Initial = function(o) {
    ...
    ;
    _this._ckall = o.ckall != null ? o.ckall : false;
    // 全选是否启用
...
layuiXtree.prototype.dataBind = function(d) {
    ...
    if (d.length > 0) {
        for (var i = 0; i < d.length; i++) {
           ...
           //wulei 修改如果是菜单没有按钮或下级菜单显示菜单图标，不显示末尾节点图标
            else  if(d[i].data.length <=0 && d[i].type!=2){
                _this._domStr += '<i class="layui-icon layui-xtree-icon-null">' + _this._iconClose + '</i>';
...
layuiXtree.prototype.Rendering = function() {
    ...
    for (var i = 0; i < xtree_items.length; i++) {
        if (xtree_items[i].parentNode == _this._container)
            ...
            if (!_this._isopen)
                xtree_items[i].style.display = 'none';
        }
    }
    ...
    _this._form.on('checkbox(xtreeck' + _this._containerid + ')', function(da) {
        ...
        for (var i = 0; i < xtree_chis.length; i++) {
            if (!_this.getChildByClassName(xtree_chis[i], 'layui-xtree-checkbox')[0].disabled) {
                ...
                if (da.elem.checked)
                    _this.getChildByClassName(xtree_chis[i], 'layui-xtree-checkbox')[0].nextSibling.classList.add('layui-form-checked');
                else
                    _this.getChildByClassName(xtree_chis[i], 'layui-xtree-checkbox')[0].nextSibling.classList.remove('layui-form-checked');
            }
        }
...
layuiXtree.prototype.GetChecked = function() {
    ...
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].checked) {
            // && cks[i].getAttribute('data-xend') == '1'
            arr[arrIndex] = cks[i];
            arrIndex++;
        }
    }
...
layuiXtree.prototype.GetParent = function(a) {
    ...
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].value == a) {
            if (cks[i].parentNode.parentNode.getAttribute('id') == _this._container.getAttribute('id'))
                return null;
